...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 30 13:11:16 2017
...
f1, f2, f3, f4 =  egnval[7:], egnvec[:,7:], egnval[7:], egnvec[:,7:]

#%%
for i in range(1000):
    x = bb + np.random.normal(scale = 0.1, size = (10, 10))
    f1, f2, f3, f4 = incremental_TD(f1, f2, f3, f4, x, 20)
    #print(f2)
#%%
np.linalg.norm(egnvec2[:,8:] @ np.diag(egnval2[8:]) @ egnvec2[:,8:].T)**2\
               / np.linalg.norm(bb)**2
